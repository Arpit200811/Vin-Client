          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField control={form.control} name="vehicleColor" render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} value={field.value} disabled={isCheckout && !isCheckin}>
                  <FormControl>
                    <SelectTrigger><SelectValue placeholder="Select color" /></SelectTrigger>
                  </FormControl>
                  <SelectContent className="max-h-60">
                    {["black","white","silver","gray","blue","red","green","yellow","brown","gold","orange","purple","beige","maroon","other"]
                      .map(color => <SelectItem key={color} value={color}>{color.charAt(0).toUpperCase() + color.slice(1)}</SelectItem>)}
                  </SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
