...
// --- NEW: TensorFlow.js Model Helper Functions ---
let model: tf.LayersModel | null = null;

async function loadModel() {
  if (!model) {
    try {
      // public/model/model.json se load karega
...
async function predictImage(image: HTMLCanvasElement) {
  if (!model) await loadModel(); // Agar model loaded nahi hai to load karo
  if (!model) {
    console.error("Prediction failed: Model not loaded.");
    return null;
  }

  // Tensor me convert karke model ke according resize karna
  const tensor = tf.browser
    .fromPixels(image)
    .resizeNearestNeighbor([224, 224]) // Model ke input size se match karein
    .toFloat()
    .expandDims(0); // Batch dimension add karna

  const prediction = model.predict(tensor) as tf.Tensor;
  const values = await prediction.data(); // Result array [0.9, 0.1] etc.

  // Memory leak se bachne ke liye Tensors ko dispose karna
  tensor.dispose();
  prediction.dispose();
...
export default function CameraScanner({
  ...
  // --- NEW: Component mount hone par model ko preload karlo ---
  useEffect(() => {
    loadModel();
  }, []);

  // --- MODIFIED: Capture logic with TensorFlow.js prediction ---
  const handleCapture = async () => {
    ...
    try {
      ...
      // --- Accurate Crop Calculation (No changes here) ---
      ...
      // --- Step 1: Predict if the image is metal ---
      const result = await predictImage(canvas);

      // result[0] > 0.5 ka matlab metal detect hua hai (aapke model ke according)
      if (result && result[0] > 0.5) {
        // --- Step 2: Metal detected, proceed with VIN scan ---
        ...
        if (!blob) {
          toast({
            title: "Failed to create image blob",
            variant: "destructive",
          });
          return;
        }
        ...
        if (response.data && response.data.vin) {
          ...
          toast({
            title: "VIN Not Found",
            description: "Could not find a VIN in the image. Please try again.",
            variant: "destructive",
          });
        }
      } else {
        // --- Not Metal: Show error and stop ---
        toast({
          title: "Not a Metal Plate",
          description: "Paper or another material was detected. Please scan the metal VIN plate.",
          variant: "destructive",
        });
      }
    } catch (error: any) {
      console.error("Capture or Prediction error:", error);
      toast({
        title: "Scan Failed",
      ...
      setIsScanning(false); // Ensure loading state is always reset
    }
  };
